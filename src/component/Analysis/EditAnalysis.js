...
const EditAnalysis = (props) => {
  const param = useParams();
  const [article, setArticle] = useState();
  const contentRef = useRef();
  const titleRef = useRef();
  useEffect(() => {
    fetch(`http://127.0.0.1:8000/api/analysis/${param.id}/`, {
      method: "Get",
      headers: {
  ...
  const changetitle = (title) => {
    ...
    fetch(`http://127.0.0.1:8000/api/analysis/title/${param.id}/`, {
        method: "PUT",
        headers: {  
          "Content-type": "application/json; charset=UTF-8",
        },
  ...
  const changecontent=(content)=>{
    ...
    fetch(`http://127.0.0.1:8000/api/analysis/content/${param.id}/`, {
        method: "PUT",
        headers: {  
          "Content-type": "application/json; charset=UTF-8",
        },
